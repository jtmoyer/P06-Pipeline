function h = f_scoreDetections(dataset, layerName, annotTimes, channels, savePrefix, dataTable)
    ...
    %   'savePrefix':   [string] prefix of layers to adds
    %
...
%     dbstop in f_scoreDetections at 136;
    ...
    allSF = [dataset.channels.sampleRate];
...
%       chanLabels{i} = sprintf('Ch_%i',varargin{2}(i));
      chanLabels{i} = dataset.channels(channels(i)).label;
    end
...
%     'FontSize',12,'Tag','decimation');  
  ...
  uicontrol(uihandle,'Style', 'pushbutton', 'Units','centimeters', 'String', 'Center Traces',...
    ...
    uicontrol(uihandle,'Style', 'pushbutton', 'Units','centimeters', 'String', 'Gain -',...
    'Position', [4.3 -1.2 2 1], 'Callback',@ZoomInY,'Parent',bottomPanel);

    uicontrol(uihandle,'Style', 'pushbutton', 'Units','centimeters', 'String', 'Zoom In',...
    'Position', [6.4 -1.2 2 1], 'Callback',@ZoomInT,'Parent',bottomPanel);
    uicontrol(uihandle,'Style', 'pushbutton','Units','centimeters', 'String', 'Zoom Out',...
    'Position', [8.5 -1.2 2 1], 'Callback',@ZoomOutT,'Parent',bottomPanel);
...
%     end
    ...
    %add Validation bar
    uicontrol(uihandle,'Style', 'pushbutton','Units','centimeters', 'String', 'Event',...
...
%     'Position', [9.7 0.1 4 1], 'BackgroundColor', [1 0 0], 'Callback',@IncorrectLbl,'Parent',bottomPanel);
    ...
    'Position', [27.4 -1.2 2 1], 'Callback',@SaveAnnot,'Parent',bottomPanel,'Tag','save');

    uicontrol(uihandle,'Style', 'pushbutton','Units','centimeters', 'String', 'Erase labels',...
    'Position', [25.3 -1.2 2 1], 'Callback',@EraseAnnot,'Parent',bottomPanel,'Tag','erase');
    ...
    annLayerIdx = strcmp(layerName,{dataset.annLayer.name});
...
%     anns=f_getAllAnnots(dataset,layerName);
...
%         labels = {'correct','uncertain','incorrect'};
        ...
        for i = 1:numel(labels)
            layerName = [setup.annLayer.name '-' labels{i}];
            foundLayer = find(strcmp(layerName,{snapshot.annLayer.name}),1);
            if ~isempty(foundLayer)
